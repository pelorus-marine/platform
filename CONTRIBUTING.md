...
- **Credits and mission** — **[`CONTRIBUTORS.md`](CONTRIBUTORS.md)** describes community attribution, **non-commercial** intent, **Rust-first** stack policy, and external library lineage (**`dbc-rs`** / **`mdf4-rs`**). **`git`** history remains authoritative for per-commit credit.

- **Pelorus first** — The **Pelorus** architecture, specs in `specifications/`, and the **`pelorus-core`** crate drive changes. External **`dbc-rs`** / **`mdf4-rs`** crates are consumed via crates.io; report issues upstream at [`sigmatactical-org`](https://github.com/sigmatactical-org).
...
- **Pinned toolchain** — **Rust 1.90.0** from [`rust-toolchain.toml`](rust-toolchain.toml). Pelorus Inspector / Tauri and shared workspace crates need one reproducible toolchain (see [`README.md`](README.md)).
...
- **Embedded-shaped checks** — extra jobs **`cargo check`** limited feature graphs (`pelorus-core` **`heapless`** / **`alloc`** without defaults) mirror on-device-ish builds.
